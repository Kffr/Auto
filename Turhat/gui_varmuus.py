...
class App:
  def __init__(self, master):
	...
	bt = subprocess.check_output("sudo /etc/init.d/bluetooth status | grep Status", shell=True)
	bttext = "Bluetooth"+bt
	self.bt1 = Label(frame2, text=bttext)
	self.bt1.pack()
	try:
		wt = subprocess.check_output("rfkill list 0 | grep yes", shell=True)
		wttext = "Wlan Down"
		...
		wttext = "Wlan up"
	self.wt1 = Label(frame2, text=wttext)
	self.wt1.pack(side=LEFT)
	def leftKey(event):
